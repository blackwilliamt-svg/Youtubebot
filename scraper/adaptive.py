...
def _fires(count: int) -> bool:
    return count > 0 and count % config.ADAPTIVE_VOTE_THRESHOLD == 0
...
def record_clip_vote(clip: dict, liked: bool) -> None:
    ...
    try:
        for tag in _clip_tags(clip):
            likes, dislikes = db.bump_tag_vote(tag, category, liked)
            if liked and _fires(likes):
                _add_search_term(tag, category, likes)
            elif not liked and _fires(dislikes):
                _remove_search_term(tag, dislikes)
    except Exception:
    ...
    if clip.get("source") == "reddit" and clip.get("subreddit"):
        try:
            name = clip["subreddit"]
            likes, dislikes = db.bump_subreddit_vote(name, liked)
            if liked and _fires(likes):
                _add_subreddit(name, category, likes)
            elif not liked and _fires(dislikes):
                _remove_subreddit(name, dislikes)
        except Exception:
